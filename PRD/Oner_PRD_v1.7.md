...
To support the heavy demands of a real-time, multi-agent dashboard, Oner relies on the following definitive tech stack:

*   **Band / Codeband:** The core orchestration framework. Powers the graph-based state sharing between all agents to prevent hallucination.
*   **Next.js:** The frontend framework for building the reactive, split-screen user dashboard.
*   **Node.js:** The backend environment responsible for executing the long-running autonomous agent loops and interacting with the LLM APIs.
*   **TailwindCSS:** For rapid styling of the dashboard GUI, strictly adhering to modern UI/UX principles.
*   **Supabase:** The database and real-time state layer. Next.js will subscribe to Supabase to animate agent conversations dynamically as they happen.
*   **Vercel:** Hosting platform for the Next.js frontend dashboard.
*   **HuggingFace:** Secondary model provider/tooling integration (used alongside Bluesminds & AI/ML API).

## 4. Agent Lineup & API Allocations
